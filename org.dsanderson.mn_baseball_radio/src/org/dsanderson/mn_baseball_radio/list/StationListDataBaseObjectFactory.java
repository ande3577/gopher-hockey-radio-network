...
public class StationListDataBaseObjectFactory implements IDatabaseObjectFactory {
	...
	private static String TYPE_NAME = "string not null";
	...
	public static String COLUMN_RANGE = "range";
	private static String TYPE_RANGE = "integer not null";
	...
	public void registerColumns(GenericDatabase database) {
		database.addColumn(COLUMN_CALLSIGN, TYPE_CALLSIGN) //
				...
				.addColumn(COLUMN_RANGE, TYPE_RANGE) //
	...
	public DatabaseObject getObject(Cursor cursor, DatabaseObject object) {
		...
		info.setRange(cursor.getLong(cursor.getColumnIndex(COLUMN_RANGE)));
		info.setDistance(cursor.getLong(cursor.getColumnIndex(COLUMN_NAME)));
